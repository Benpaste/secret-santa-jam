...
func spawn_bullet_ring(amount := 40, position_ := Vector2.ZERO, speed := Bullet.BASE_SPEED, direction_offset := 0.0, wait_time := 0.0, scene := bullet_scene) -> void:
	for i in amount:
		spawn_bullet((2*i*PI/amount) + direction_offset, speed, position_, false, scene)
